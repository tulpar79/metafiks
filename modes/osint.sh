if [[ "$OSINT" = "1" ]]; then
	echo "[metafikssecurity.com] •?((¯°·._.• Started metafiks OSINT scan: $TARGET [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
	if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
		/bin/bash "$INSTALL_DIR/bin/slack.sh" "[metafikssecurity.com] •?((¯°·._.• Started metafiks OSINT scan: $TARGET [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
	fi	
	if [[ "$WHOIS" == "1" ]]; then
