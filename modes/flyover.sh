...
if [[ "$MODE" = "flyover" ]]; then
  ...
  if [[ "$REPORT" = "1" ]]; then
    ...
    echo "[metafiks.com] •?((¯°·._.• Started metafiks scan: $FILE [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
    if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
      /bin/bash "$INSTALL_DIR/bin/slack.sh" "[metafiks.com] •?((¯°·._.• Started metafiks scan: $FILE [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
    fi
    args=""
    ...
    if [[ -s "$LOOT_DIR/nmap/livehosts-sorted.diff" ]]; then
      echo "[metafiks.com] •?((¯°·._.• Host status change detected on $TARGET (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
      ...
      if [[ "$SLACK_NOTIFICATIONS_NMAP_DIFF" == "1" ]]; then
        /bin/bash "$INSTALL_DIR/bin/slack.sh" "[metafiks.com] •?((¯°·._.• Host status change detected on $WORKSPACE (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
    ...
    do
      ...
      if [[ -s "$LOOT_DIR/nmap/ports-$a.diff" ]]; then
        echo "[metafiks.com] •?((¯°·._.• Port change detected on $a (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
        cat $LOOT_DIR/nmap/ports-$a.diff | egrep "<|>" >> $LOOT_DIR/scans/notifications_new.txt
        if [[ "$SLACK_NOTIFICATIONS_NMAP_DIFF" == "1" ]]; then
          /bin/bash "$INSTALL_DIR/bin/slack.sh" "[metafiks.com] •?((¯°·._.• Port change detected on $a (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
    ...
    ls -lh $LOOT_DIR/scans/running_*.txt 2> /dev/null | wc -l 2> /dev/null > $LOOT_DIR/scans/tasks-running.txt
    echo -e "${OKGREEN}====================================================================================${RESET}•x${OKGREEN}[`date +"%Y-%m-%d](%H:%M)"`${RESET}x•"
    echo "[metafiks.com] •?((¯°·._.• Finished metafiks scan: $FILE [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•" >> $LOOT_DIR/scans/notifications_new.txt
    if [[ "$SLACK_NOTIFICATIONS" == "1" ]]; then
      /bin/bash "$INSTALL_DIR/bin/slack.sh" "[metafiks.com] •?((¯°·._.• Finished metafiks scan: $FILE [$MODE] (`date +"%Y-%m-%d %H:%M"`) •._.·°¯))؟•"
    fi
    if [[ "$LOOT" = "1" ]]; then
      loot
    fi
