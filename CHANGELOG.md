...
* v9.1 - Updated default aux mode options in default metacy.conf
...
* v9.0 - Renamed AUTO_VULNSCAN setting to "VULNSCAN" in metacy.conf to perform vulnerability scans via 'normal' mode
* v8.9 - Tuned metacy.conf around performance for all scans and recon modes
...
* v8.8 - Added static grep searching rules of all URL's and sub-domains (see metacy.conf for details)
* v8.8 - Added verbose status logging to flyover mode showing HTTP status/redirect/title, etc.
* v8.8 - Added integration for Port Scanner Add-on for metafiks Professional
...
* v8.6 - Added new metafiks configuration flow that allows persistent user configurations and API key transfer
...
* v8.5 - Removed larger wordlists to reduce install size of metafiks
...
* v8.4 - Added 8 new metafiks configuration templates (see /usr/share/metacy/conf/)
...
* v8.4 - Disabled legacy Metasploit web exploits (check metafiks conf to re-enable)
* v8.4 - Fixed issue with dirsearch asterisk being used incorrectly
* v8.4 - Fixed issue with airstrike mode not updated metafiks Professional v8.0 host list
...
* v8.3 - Added NMAP_OPTIONS setting to metacy.conf to configure optional NMap scan settings
* v8.3 - Added option to specify custom metafiks configuration via (-c) switch
...
* v8.3 - Added flyover mode tuning options to metacy.conf
...
* v8.3 - Added metacy.conf options for OPENVAS_HOST and OPENVAS_PORT selection for remote instances
* v8.3 - Improved 'vulnscan' mode via OpenVAS to scan the same asset multiple times with improved error handling
* v8.2 - Added root priv check to metacy script to run
...
* v8.2 - Added Kali start menu app & icon for metafiks
* v8.2 - Added check for insecure SSL/TLS connections
* v8.2 - Added NMAP_OPTIONS setting in ~/.metacy.conf to configure optional NMap settings
...
* v8.1 - Updated metafiks Professional autoload settings
...
* v8.1 - Fixed issue with -dh flag to delete host with metafiks Pro v8.0
...
* v7.1 - Fixed/added Metasploit LHOST/LPORT values to all exploits based on metacy.conf settings
...
* v7.0 - Added Slack API notifications (Disabled by default..check ~/.metacy.conf)
* v7.0 - Added new command switch to add daily, weekly or monthly metacy scheduled scans... check README
...
* v7.0 - Added Gobuster (Disabled by default..check ~/.metacy.conf)
...
* v6.2 - Added DockerFile to run metafiks in Docker (CC. Hariom Vashisth <hariom.devops@gmail.com>)
* v6.2 - Changed option to automatically import all NMap XML's into Metasploit's DB
* v6.2 - Changed option to automatically load metafiks Professional's report when scans complete
* v6.2 - Added config option to enable/disable subdomain hijacking checks in metacy.conf
* v6.2 - Fixed issue with metacy --list command having invalid reference
...
* v6.1 - Added configuration options to set LHOST/LPORT for all Metasploit exploits in metacy.conf
...
* v6.1 - Added new metacy.conf options to enabled/disable all plugins and change settings per user
...
* v6.1 - Fixed issue with metafiks Professional notepad.html missing
...
* v5.7 - Added metacy.conf scan configuration file to customize metacy environments by user
...
* v5.6 - Changed automatic report generation to "ON" for metafiks Pro users
* v5.5 - Added new multi-threaded high speed "flyover" mode added
* v5.5 - Added new scan status mode via (metacy --status) command
...
* v5.2 - Added configurable modules/plugins to metacy script
...
* v5.1 - Added dnscan to install.sh and updated metacy references which were broken
...
* v5.0 - Added metafiks Pro reporting interface (see https://metafikssecurity.com for more details)
...
* v4.4 - Fixed issue with metacy nuke and airstrike modes not running.
* v4.4 - Added improved SNMP checks via NMap/Metasploit.
* v4.4 - Resolved dependency issue for nfs-common package.
* v4.4 - Fixed bug in metacy -fp command switch.
* v4.3 - Fixed bug in version info.
* v4.2 - Fixed bad merge in 4.1 causing metacy to break.
* v4.1 - Fixed a few bugs with various command line switches for airstrike and nuke modes.
* v4.1 - Fixed issue with path relative file inclusion via the -f flag. You can now include just the local filename (metacy -f targets.txt).
* v4.0 - Added new command switch options for all metacy scans (see --help for details)
...
* v3.0 - Improved performance of various metacy modes
...
* v2.9 - Created uninstall.sh script to uninstall metacy
...
* v2.7 - Fixed issue with metacy update command and install.sh not running
* v2.7 - Fixed errors with GooHak
* v2.7 - Fixed syntax errors in metacy conditional statements 
...
* v2.5f - Fixes for various bugs reported and fixed by @ifly53e (https://github.com/1N3/metafiks/pull/89)
...
* v2.5 - Added HTML report generation via metacy 'loot' command
* v2.5 - Added automatic NMap searchsploit integration to find exploits
* v2.5 - Added various improvements to metafiks discovery scan mode
...
* v2.2b - Fixed minor issue with metacy loot command
* v2.2a - Fixed minor issue with loot function
* v2.2 - Added auto Metasploit Pro & Zenmap GUI integration
* v2.2 - Added metafiks workspaces to loot directory
...
* v1.9 - Added Ubuntu docker image for metafiks (@menzow)
...
* v1.8c - Added install directory (/usr/share/metacy/) to install script for universal access
...
* v1.6 - Added option to set metafiks full path for universal command line access
...
* v1.5c - Fixed issue which broke link to metacy directory
...
* v1.4e - Added auto-upgrade option to install.sh for existing metafiks installs
